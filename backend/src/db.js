...
async function initDB() {
    ...
    try {
      ...
      // ── LAYER 1: CONTRACTOR-LEVEL MEMORY (business prefs, global facts) ──────
      await client.query(`
                                            ...
                                            key TEXT NOT NULL,
                                                    value TEXT NOT NULL,
                                                            category TEXT DEFAULT 'general',
                                                                    created_at TIMESTAMPTZ DEFAULT NOW(),
                                                                            updated_at TIMESTAMPTZ DEFAULT NOW(),
                                                                                    UNIQUE(contractor_id, lead_phone, key)
                                                                                          )
                                                                                              `);

      -- Add lead_phone column to existing memory tables (safe migration)
          await client.query(`
                ALTER TABLE memory ADD COLUMN IF NOT EXISTS lead_phone TEXT
                    `);

      -- Fix unique constraint to include lead_phone
          await client.query(`
                DO $$
                                   BEGIN
                                           IF EXISTS (
      ...
      // ── LAYER 2: LEADS (per-lead records) ────────────────────────────────────
      await client.query(`
      ...
      // ── LAYER 3: JOBS (episodic job-level memory) ─────────────────────────────
      await client.query(`
      ...
      console.log('[DB] Schema initialized successfully — 3-layer memory active');
    } finally {
          client.release();
    }
}
