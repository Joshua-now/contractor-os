...
async function requireAuth(req, res, next) {
  ...
  try {
    ...
    const { rows: [contractor] } = await pool.query(
      'SELECT id, name, company_name, email, phone, telnyx_phone, sms_provider, plan, role, active, bob_enabled, ai_persona, service_area, services, review_link, ghl_location_id FROM contractors WHERE id = $1 AND active = true',
