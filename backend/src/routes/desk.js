...
router.post('/chat', async (req, res) => {
      try {
        ...
        // Joshua's contractor ID is always looked up by phone (the owner account)
        const { rows: [cRow] } = await pool.query("SELECT id FROM contractors WHERE phone = '+13212055991' LIMIT 1");
      ...
      const results = {};
        ...
        try {
             ...
             try {
                     ...
                     const campaigns = instantly.data?.campaigns || instantly.data || [];
                     results.instantly = {
                               ok: true,
